...
def read_category(name:str):
  ...
  category = session.query(CategoriesModel).filter(CategoriesModel.name==name.title()).first()
...
def update_category(name:str):
  ...
  selected_category = session.query(CategoriesModel).filter(CategoriesModel.name == name.title()).first()
...
def delete_category(name):
  ...
  selected_category = selected_category = session.query(CategoriesModel).filter(CategoriesModel.name == name.title()).first()
  if not selected_category:
    return {'Error':'Collection not found'}, 404
